...
export default function PrescriptionConsole() {
  ...
  function handleSaveAsTemplate() {
    const templateName = window.prompt("Name this template:", diagnosis || "New Template");
  ...
  function handleFinalize() {
    const hasValidMedicine = medicines.some((med) => med.drugName.trim() !== "");
    if (!diagnosis.trim() || !hasValidMedicine) {
      setStatusMessage("Add a diagnosis and at least one medicine before finalizing.");
      setTimeout(() => setStatusMessage(""), 3000);
      return;
    }
    setIsFinalized(true);
  ...
  return (
    <div>
      ...
      {isFinalized && (
        <div className="flex items-center gap-2.5 bg-mint-light border border-emerald-200 rounded-lg px-4 py-3 mb-6">
          <CheckCircle2 size={18} className="text-emerald-600 shrink-0" />
          <p className="text-[13.5px] text-emerald-700 font-semibold">
            Prescription finalized and saved to patient record.
          </p>
        </div>
      )}
      ...
      {statusMessage && !isFinalized && (
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2.5 mb-6">
          <p className="text-[13px] text-amber-700 font-medium">{statusMessage}</p>
        </div>
      )}

      <fieldset disabled={isFinalized} className="disabled:opacity-60">
        <div className="grid lg:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 h-fit">
            ...
            <div className="grid grid-cols-[1fr_90px_130px_28px] gap-3 text-[10.5px] font-bold uppercase text-slate-400 mb-2 px-1">
              <span>Drug Name</span>
              <span>Dosage</span>
              <span>Frequency</span>
              <span></span>
            ...
            <div className="flex flex-col gap-2">
              {medicines.map((med) => (
                <PrescriptionMedicineRow
                  ...
                  onRemove={removeDrugRow}
                />
              ))}
              {medicines.length === 0 && (
                <p className="text-slate-400 text-[13px] text-center py-4">No medicines added yet.</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 border border-slate-300 text-slate-700 font-medium text-[13.5px] px-4 py-2.5 rounded-lg hover:bg-slate-50 transition-colors"
          >
            <Printer size={15} />
            Print Draft
          </button>
          <button
            ...
            className="flex items-center gap-2 border border-slate-300 text-slate-700 font-medium text-[13.5px] px-4 py-2.5 rounded-lg hover:bg-slate-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <button
          ...
          disabled={isFinalized}
          className="flex items-center gap-2 bg-brand hover:bg-brand-dark text-white font-semibold text-[13.5px] px-5 py-3 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          <CheckCircle2 size={16} />
          {isFinalized ? "Prescription Finalized" : "Save & Finalize Prescription"}
        </button>
      </div>
    </div>
  );

...
import { Download, PlusCircle, Users, UserPlus, Wallet, CalendarClock, Ticket, Link2, AlertTriangle, Check } from "lucide-react";
import { useNavigate } from "react-router-dom";
import AdminKPICard from "../components/functions/AdminKPICard.jsx";
import { adminKpis as KPIS, approvals as INITIAL_APPROVALS, queueRooms as QUEUE_ROOMS } from "../data/allData";
...
export default function AdminDashboard() {
  ...
  function handleExportReport() {
    const header = "Patient Name,ID,Doctor,Department,Type,Action\n";
    const rows = approvals
      .map((row) => `${row.name},${row.id},${row.doctor},${row.dept},${row.type},${row.action}`)
      .join("\n");
  ...
  function handleApprovalAction(id) {
      ...
      <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex items-center gap-2 bg-slate-100 hover:bg-slate-200 text-ink font-medium text-[13.5px] px-4 py-2.5 rounded-lg transition-colors"
          >
            <Download size={15} />
            Export Report
          </button>
              ...
              <tbody className="divide-y divide-slate-100">
                {approvals.map((row) => (
                  <tr key={row.id}>
                    ...
                    <td className="px-4 py-3.5">
                      <button
                        onClick={() => handleApprovalAction(row.id)}
                        className="flex items-center gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-[12.5px] font-semibold px-3 py-2 rounded-lg transition-colors"
                      >

...
import { roleNavItems } from "../../data/allData";
...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 shrink-0 bg-white border-r border-slate-200 px-4 py-6">
      ...
      <nav className="flex flex-col gap-1">
        {navItems.map(({ label, icon: Icon, path }) => (
          <NavLink
            key={path}
            to={path}
            end={path === "/dashboard" || path === "/dashboard/appointments" || path === "/dashboard/admin"}
            className={({ isActive }) =>
      ...
      {/* Only Patients & Doctors do video consultations — hide this for Admin */}
